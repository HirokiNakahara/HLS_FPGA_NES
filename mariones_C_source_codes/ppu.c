...
// Vivado HLSで合成するときは上位4b x 3 = 12bで
...
void ppu_render( int line, unsigned char *bmp)
{
  ...
  // １行の描画データを バッファ(bmp)に転送
	for ( i = 0; i < 256; i++){
    //nes_screen[i][line]=nes_palette_24[ p[i] & 0x3f];
    
    if( ((239 - line) * 256 * 3 + i * 3) < 0){
    	printf( "ERROR EXCEED SCREEN BUFFER\n");
    	exit(-1);
    }
    
    // BMP画像形式で出力
    // この配列を直接 fwrite すればBMP画像を出力できる (ヘッダもいるけど…)
    bmp[ (239 - line) * 256 * 3 + i * 3]     = pal_dat[ p[i] & 0x3f][ 2]; // Red
    bmp[ (239 - line) * 256 * 3 + i * 3 + 1] = pal_dat[ p[i] & 0x3f][ 1]; // Green
    bmp[ (239 - line) * 256 * 3 + i * 3 + 2] = pal_dat[ p[i] & 0x3f][ 0]; // Blue
	}
}
