...
// NES本体
int mariones_top( 
	int reset, // ソフトウェアリセット
	unsigned char bmp[256*3*240], // 画面描画。これをBMPに変換する
	int pad[2][8] //ジョイパッドの状態, [プレイヤ番号][各ボタン], 1にすると押した状態
);
...
int main()
{
	...
	unsigned char bmp[PIC_DATA_SIZE];
	...
	for( cnt = 0; cnt < max_frame; cnt++){
		...
		pad_dat[0][0] = mov[cnt] & 0x1;
		pad_dat[0][1] = ( mov[cnt] >> 1) & 0x1;
		pad_dat[0][2] = ( mov[cnt] >> 2) & 0x1;
		pad_dat[0][3] = ( mov[cnt] >> 3) & 0x1;
		pad_dat[0][4] = ( mov[cnt] >> 4) & 0x1;
		pad_dat[0][5] = ( mov[cnt] >> 5) & 0x1;
		pad_dat[0][6] = ( mov[cnt] >> 6) & 0x1;
		pad_dat[0][7] = ( mov[cnt] >> 7) & 0x1;
